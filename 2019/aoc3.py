...
minsteps    = 9999999999999

def parse_wire(wirestring, wire_id):
	global posmap, mindistance, minsteps

	posX = 0
	posY = 0
	steps = 0

	for movement in wirestring.split(','):
		direction = movement[0]
		count = int(movement.strip('RLUD'))
		...
		for i in range(0, count):
			...
			#print(posstr)
			if posmap.get(posstr) == None:
				posmap[posstr] = {
					"id": wire_id,
					"steps": steps
				}
			else:
				if posmap[posstr]["id"] == wire_id:
					# crossing myself -> ignore
					continue

				distance = abs(posX) + abs(posY)
				totalsteps = steps + posmap[posstr]["steps"]

				print("intersection at (%d, %d) distance %d, steps(this)=%d, total steps %d"
					% (posX, posY, distance, steps, totalsteps))
...
parse_wire(wire1, 'wire1')

print

parse_wire(wire2, 'wire2')

print("mindistance: %d" % mindistance)
print("minsteps: %d" % minsteps)
